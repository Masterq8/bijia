...
//    private final ExecutorService executorService = Executors.newFixedThreadPool(5);
    ...
    public AjaxResult hutai2Detection(Long id, SysUser account) throws Exception {

        String workplace = "C:/hutai-2/";

        String ID = IDGrn(account.getUserName(),null);

        String workplaceWin = "C:/hutai-2/";
        // 海水养殖区
        // xml文件信息读取，读取路径需自定义
...
//        info2DB.setState("2");
        ...
        info2DB.setOrderName("浒苔2m分辨率斑块提取");
        info2DB.setOrderDetail("待处理数据"+data.getSourcefilename().substring(0,data.getSourcefilename().length()-5)+"浒苔2m分辨率斑块提取操作");
...
//                }
                new hutai2Router().processSubprocessBuffer(process2, logger);
