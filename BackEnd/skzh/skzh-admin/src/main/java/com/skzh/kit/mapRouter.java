...
public class mapRouter {
    ...
    private static final Logger logger = Logger.getLogger(com.skzh.kit.mapRouter.class.getName());
    ...
    public AjaxResult zhuantituGen(zhuantituVo data, SysUser account) throws Exception {
        ...
        Satellite satellite = satelliteMapper1.selectSatelliteById(Long.valueOf(orderInfo.getMapid()));
...
//        String[] imgDate = data.getStarttime().toString().split("-");
        ...
        data2DB.setCategory("GTM");
        data2DB.setMapid(data.getId());
        data2DB.setOrderTime(String.valueOf(LocalDateTime.now()));
        data2DB.setState("0");
        data2DB.setUserId(account.getUserId());

        String shpurl="F:/Results/"+str+"/"+"GF1"+"/"+satellite.getId()+"/"+satellite.getSourcefilename().replace(".tiff","")+"/coastline.shp";
        data2DB.setShpurl(shpurl);
...
//        );
        String pythonPath = "D:/anaconda3/envs/arcgispro-py3-clone/python.exe";
        ProcessBuilder processBuilder = new ProcessBuilder(
                pythonPath,
                "D:/bijia/Arcpypro/main.py",
                "--title", data.getTitle(),
                "--satellite", satellite.getSatelliteType(),
                "--sensor", satellite.getSensorType(),
                "--resolution", satellite.getResolution(),
                "--date", imgDate1,
                "--template", "1", // 注意这里应该是字符串，而不是数字
                "--sourcePath", satellite.getSourcefilepath(),
                "--dataPath", data2DB.getShpurl(),
                "--ID", String.valueOf(satellite.getId()), // 确保 ID 是字符串
                "--outputDir", outputPath + data2DB.getAccount() + "/map"
        );
        ...
        Thread taskThread = new Thread(() -> {
            try {
                processBuilder.directory(new File("D:/bijia/Arcpypro"));
                Process process = processBuilder.start();
                //处理标准输出
                new com.skzh.kit.mapRouter().processSubprocessBuffer(process, logger);
                int exitCode = process.waitFor();
                System.out.println(exitCode);
                if (exitCode == 0) {

                    // Python脚本成功执行
                    String thumbnail = "F:/Results/"+str+"/map/" + satellite.getId() + "/" + "pic/" + satellite.getId() + ".jpg";
                    String filePath = "F:/Results/"+str+"/map/" + satellite.getId() + "/" + "pic/"+ satellite.getId() + ".png";
