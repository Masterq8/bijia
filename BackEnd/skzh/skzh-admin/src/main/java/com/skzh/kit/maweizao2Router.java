...
//    private final ExecutorService executorService = Executors.newFixedThreadPool(5);
    ...
    public AjaxResult mawizao2Detection(Long id, SysUser account) throws Exception {

        String workplace = "E:/GF6/GF6Project/";

        String ID = IDGrn(account.getUserName(),null);

        String workplaceWin = "E:/GF6/GF6Project/";
        // 海水养殖区
        // xml文件信息读取，读取路径需自定义
...
//        info2DB.setState("2");
        ...
        info2DB.setOrderName("马尾藻2m分辨率斑块提取");
        info2DB.setOrderDetail("待处理数据"+data.getSourcefilename().substring(0,data.getSourcefilename().length()-5)+"马尾藻2m分辨率斑块提取操作");
...
//        );
        ...
        String scriptPath = "Detection.py";
        ProcessBuilder processBuilder = new ProcessBuilder(
                pythonPath,
                scriptPath
        );
...
//                }
                new maweizao2Router().processSubprocessBuffer(process2, logger);
