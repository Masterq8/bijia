...
//    private final ExecutorService executorService = Executors.newFixedThreadPool(5);
    ...
    public AjaxResult maweizaoDetection(Long id, SysUser account) throws Exception {

        String workplace = "F:/maweizao-16/";

        String ID = IDGrn(account.getUserName(),null);

        String workplaceWin = "F:/maweizao-16/";
        // 海水养殖区
        // xml文件信息读取，读取路径需自定义
...
//        info2DB.setState("2");
        ...
        info2DB.setOrderName("马尾藻斑块提取");
        info2DB.setOrderDetail("待处理数据"+data.getSourcefilename().substring(0,data.getSourcefilename().length()-5)+"马尾藻斑块提取操作");
...
//                }
//                new maweizaoRouter().processSubprocessBuffer(process2, logger, info2DB.getId(), account.getUserName());
                new Thread(() -> {
...
//            }
                ...
                if (code == 0) {
                    ...
                    String zipFilePath = outputPath + account.getUserName() + "/" + data.getSatelliteType() + "/" + data.getId() + "/" + data.getSourcefilename().substring(0,data.getSourcefilename().length()-5)+"_1" + ".zip";
                    info2DB.setFilepath(zipFilePath);
                    System.out.println(info2DB.getFilepath());

                    try {
                        ZipUtil.zip(outputPath + account.getUserName() + "/" + data.getSatelliteType() + "/" + data.getId() + "/" + data.getSourcefilename().substring(0,data.getSourcefilename().length()-5)+"_1", info2DB.getFilepath());
//                        ZipUtil.zip(outputPath + account.getUserName() + "/" + data.getSatelliteType() + "/" + data.getId() + "/1" , info2DB.getFilepath());
                        log.info("打包完成");
                    } catch (Exception e) {
                        log.error("打包失败", e);
                    }
