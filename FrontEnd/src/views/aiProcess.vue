      ...
      <div class="tool-list">
        <section v-for="group in toolGroups" :key="group.title" class="tool-section">
          ...
          <div class="tool-grid">
            ...
            >
              <span v-if="tool.beta" class="tool-card__lock">🔒</span>
            ...
            <template #empty>
              <div class="empty-state">
                ...
                <div class="empty-state__actions">
                  <el-button type="primary" size="large" @click="goHome">去选择/上传影像</el-button>
                  <el-button size="large" @click="showExampleGuide">查看示例流程</el-button>
                </div>
              </div>
            </template>
          </el-table>
...
.tool-card {
  min-height: 142px;
  padding: 10px;
  border: 1.8px solid var(--tool-card-border);
  border-radius: 14px;
  background: var(--card-bg);
  color: var(--text);
  cursor: pointer;
  ...
  &.is-active {
    background: linear-gradient(180deg, var(--primary-soft), var(--card-bg));
    border-color: var(--primary);
    box-shadow: 0 0 0 2px rgba(79, 125, 243, 0.08), var(--shadow-lg);
  }
}
